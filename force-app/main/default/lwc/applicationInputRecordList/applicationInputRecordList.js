...
export default class ApplicationInputRecordList extends LightningElement {
	...
	@wire(getObjectInfo, { objectApiName: '$childsObjectApi' })
	...
	get recordTypeId() {
		...
		return Object.keys(rtis).find((rti) => rtis[rti]?.name === this.childsObjectRecordTypeAPIName)
	}
	...
	get childsObjectRecordTypeAPIName() {
		return this.detail?.Child_sObject_Record_Type_API_Name__c || ''
	}
	...
	get allowDelete() {
		...
		if (this.data.length === 1 && this.detail?.Allow_Delete__c && this.required) {
			return false
		}
	...
	async fetchFieldSetData() {
		this.data = await getFieldSetData({
			...
			recordTypeAPIName: this.childsObjectRecordTypeAPIName
		});
		// console.log("DATA");
